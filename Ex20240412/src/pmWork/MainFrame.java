...
public class MainFrame extends JFrame {
	...
	String path = "C:/My_Study/io_table_test.txt";
	File f;
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(cmd != ADD) {
					...
					if(empno.length()==0) {
						JOptionPane.showMessageDialog(MainFrame.this, "사번을 입력하세요.");
						return;
					} else if(ename.length()==0) {
						JOptionPane.showMessageDialog(MainFrame.this, "이름을 입력하세요.");
						return;
					} else if(dept.length()==0) {
						JOptionPane.showMessageDialog(MainFrame.this, "부서를 입력하세요.");
						return;
					} else if(pos.length()==0) {
						JOptionPane.showMessageDialog(MainFrame.this, "직책을 입력하세요.");
						return;
					} else if(hiredate.length()==0) {
						JOptionPane.showMessageDialog(MainFrame.this, "입사일을 입력하세요.");
						return;
					} 
					
					// 받은 값들을 하나의 객체화
					EmpVO vo = new EmpVO(empno, ename, pos, dept, hiredate);
					
					list.add(vo);
					
					// 저장완료의 의미로 메시지 창 보여주기
					JOptionPane.showMessageDialog(MainFrame.this, "저장 완료");
	...
	private void viewTotal() {
		...
		// JTextArea에 출력
		empArr = new String[list.size()][5];

		
		for(int i=0; i<list.size();i++) {
			EmpVO vo = list.get(i); // 요소 하나 얻어내기
			
			empArr[i][0] = vo.getEmpno();
			empArr[i][1] = vo.getEname();
			empArr[i][2] = vo.getDept();
			empArr[i][3] = vo.getPos();
			empArr[i][4] = vo.getHireDate();
		} // for문의 끝
		
		// sb가 가지고 있는 문자열의 집합을 JTextArea에 표현
		table.setModel(new DefaultTableModel(empArr,c_name));
